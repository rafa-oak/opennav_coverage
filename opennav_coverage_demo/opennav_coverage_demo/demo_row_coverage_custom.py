...
class RowCoverageNavigatorTester(Node):

    def __init__(self):
        super().__init__(node_name='coverage_row_navigator_tester')
    ...
    def navigateCoverage(self, field_file):
        ...
        goal_msg.behavior_tree = get_package_share_directory('opennav_coverage_bt') + \
            '/behavior_trees/navigate_w_basic_row_complete_coverage.xml'

        print('Navigating to with field file: ' + str(field_file) + '...')
...
def main():
    ...
    navigator = RowCoverageNavigatorTester()
    navigator.startup()

    # Some example field
    field = get_package_share_directory('opennav_coverage') + '/irregular_test_field.xml'
